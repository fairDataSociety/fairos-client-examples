...
function App() {
  ...
  async function connectWallet() {
    if (window.ethereum) {
      ...
      try {
        ...
        let resp = await window.connectWallet("check", "123123123123123", web3.eth.defaultAccount, signature)
        console.log(resp)
      } catch (error) {
          console.error(error);
      }
  ...
  async function loginWallet() {
    if (window.ethereum) {
      ...
      try {
        ...
        const signature = await web3.eth.personal.sign(web3.eth.defaultAccount, web3.eth.defaultAccount, '')
        console.log(signature)
        let resp = await window.walletLogin(web3.eth.defaultAccount, signature)
        console.log(resp)
  ...
  async function mConnect() {
    let respConnect = await window.connect("", "", "", "testnet", "", "")
    console.log(respConnect)


    let respLogin = await window.login("asdasd", "asdasdasdasd")
